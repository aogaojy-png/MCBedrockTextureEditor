...
public class MainActivity extends android.app.Activity {
    ...
    private static final int BG_PICK_IMAGE = 4001;
    private static final int SYSTEM_CROP_REQUEST = 5001;

    // 主题模式
    private static final int THEME_LIGHT = 0;
    private static final int THEME_DARK = 1;
    private static final int THEME_CUSTOM = 2;
    private int themeMode = THEME_LIGHT;

    // UI 常量
    private static final float TEXT_TITLE = 26f;
    private static final float TEXT_HEADING = 20f;
    private static final float TEXT_BODY = 14f;
    private static final float TEXT_SMALL = 12f;
    private static final float TEXT_LABEL = 11f;
    private static final float TEXT_BUTTON = 15f;
    private static final float TEXT_ICON = 23f;
    ...
    // 粘性筛选器（吸顶）相关
    private LinearLayout contentFilterContainer;
    private LinearLayout stickyFilterContainer;
    ...
    private static final int LIME = Color.rgb(139, 195, 74);
    private static final int MINT = Color.rgb(0, 200, 150);
...
private class ImportTask extends AsyncTask<Void, Integer, ArrayList<Entry>> {
    ...
    private int processedPngFiles = 0;
    ...
    protected ArrayList<Entry> doInBackground(Void... voids) {
        try {
            ...
            totalPngFiles = countPngFiles(extractDir);
            if (totalPngFiles == 0) {
                return new ArrayList<>();
            }

            Map<String, String> textureKeyToPath = new HashMap<>();
            Map<String, String> pathToTextureKey = new HashMap<>();

            File itemJson = new File(extractDir, "textures/item_texture.json");
            if (itemJson.exists()) {
                try {
                    String content = new String(java.nio.file.Files.readAllBytes(itemJson.toPath()), StandardCharsets.UTF_8);
                    JSONObject root = new JSONObject(content);
                    JSONObject textureData = root.optJSONObject("texture_data");
                    if (textureData != null) {
                        Iterator<String> keys = textureData.keys();
                        while (keys.hasNext()) {
                            String key = keys.next();
                            JSONObject entry = textureData.getJSONObject(key);
                            String textures = entry.optString("textures");
                            if (textures != null && !textures.isEmpty()) {
                                textureKeyToPath.put(key, textures);
                                pathToTextureKey.put(textures, key);
                            }
                        }
                    }
            ...
            File terrainJson = new File(extractDir, "textures/terrain_texture.json");
            if (terrainJson.exists()) {
                try {
                    String content = new String(java.nio.file.Files.readAllBytes(terrainJson.toPath()), StandardCharsets.UTF_8);
                    JSONObject root = new JSONObject(content);
                    JSONObject textureData = root.optJSONObject("texture_data");
                    if (textureData != null) {
                        Iterator<String> keys = textureData.keys();
                        while (keys.hasNext()) {
                            String key = keys.next();
                            JSONObject entry = textureData.getJSONObject(key);
                            JSONArray texturesArr = entry.optJSONArray("textures");
                            if (texturesArr != null && texturesArr.length() > 0) {
                                String textures = texturesArr.getString(0);
                                textureKeyToPath.put(key, textures);
                                pathToTextureKey.put(textures, key);
                            }
                        }
            ...
            if (texturesDir.exists()) {
                scanTexturesWithMapping(texturesDir, "", imported, pathToTextureKey);
            }
                ...
                if (subFolders != null) {
                    for (File subFolder : subFolders) {
                        ...
                        if (subTextures.exists()) {
                            scanTexturesWithMapping(subTextures, subName, imported, pathToTextureKey);
                        }
                    }
                    saveSubpackList();
                }
    ...
    protected void onProgressUpdate(Integer... values) {
        if (values.length > 0) {
            ...
            if (values[0] < 10) progressDialog.setMessage("正在解压...");
            else if (values[0] < 30) progressDialog.setMessage("正在扫描纹理映射...");
            else if (values[0] < 70) progressDialog.setMessage("正在处理纹理文件...");
    ...
    private void scanTexturesWithMapping(File dir, String subpack, ArrayList<Entry> out,
                                         Map<String, String> pathToTextureKey) {
        File[] files = dir.listFiles();
        if (files == null) return;
        for (File f : files) {
            if (f.isDirectory()) {
                scanTexturesWithMapping(f, subpack, out, pathToTextureKey);
            } else if (f.getName().toLowerCase().endsWith(".png")) {
                String absolutePath = f.getAbsolutePath();
                String relativePath = null;
                int texturesIndex = absolutePath.indexOf("textures" + File.separator);
                if (texturesIndex != -1) {
                    String afterTextures = absolutePath.substring(texturesIndex + "textures".length() + 1);
                    int extIndex = afterTextures.lastIndexOf('.');
                    if (extIndex != -1) {
                        relativePath = afterTextures.substring(0, extIndex);
                    }
                }
                if (relativePath == null) {
                    relativePath = f.getName().replace(".png", "");
                }

                String textureKey = null;
                if (pathToTextureKey != null && pathToTextureKey.containsKey(relativePath)) {
                    textureKey = pathToTextureKey.get(relativePath);
                }
                if (textureKey == null) {
                    String fileName = f.getName().replace(".png", "");
                    for (Map.Entry<String, String> entry : pathToTextureKey.entrySet()) {
                ...
                if (textureKey != null) {
                    ...
                    if (parentPath != null) {
                        if (parentPath.contains("blocks") || parentPath.contains("block")) type = "方块";
                        else if (parentPath.contains("entity") || parentPath.contains("mobs")) type = "生物";
                        else if (parentPath.contains("environment") || parentPath.contains("sky")) type = "天空";
                        else if (parentPath.contains("ui")) type = "UI";
                    }
                } else {
                    String parent = dir.getAbsolutePath();
                    String base = new File(parent).getParent();
                    if (base == null) base = parent;
                ...
                processedPngFiles++;
                int progress = 10 + (int) ((float) processedPngFiles / totalPngFiles * 85f);
                publishProgress(Math.min(95, progress));
            }
        }
    }

    private int countPngFiles(File dir) {
        int count = 0;
        File[] files = dir.listFiles();
        if (files == null) return 0;
        for (File f : files) {
            if (f.isDirectory()) {
                count += countPngFiles(f);
            } else if (f.getName().toLowerCase().endsWith(".png")) {
                count++;
            }
        }
...
protected void onCreate(Bundle b) {
    super.onCreate(b);
    initImageCache();
    themeMode = getPreferences(MODE_PRIVATE).getInt("theme_mode", THEME_LIGHT);
    darkMode = (themeMode == THEME_DARK);
    loadSubpackList();

    String bgUriStr = getPreferences(MODE_PRIVATE).getString("bg_uri", "");
    if (!bgUriStr.isEmpty()) {
        try {
            backgroundImageUri = Uri.parse(bgUriStr);
            getContentResolver().takePersistableUriPermission(backgroundImageUri,
...
private void cleanOldTempImages() {
    ...
    if (files != null) {
        for (File f : files) {
            if (f.isFile() && (now - f.lastModified()) > expireTime) {
                f.delete();
            }
        }
    }
...
private void buildShell() {
    ...
    // 真正的底层背景：图片永远在最下面，页面 UI 永远叠在上面
    backgroundView = new ImageView(this);
    backgroundView.setScaleType(ImageView.ScaleType.CENTER_CROP);
    backgroundView.setAdjustViewBounds(false);
    backgroundView.setBackgroundColor(bg());
    if (themeMode == THEME_CUSTOM && backgroundImageUri != null) {
        loadBackgroundIntoView(backgroundImageUri);
    }
    FrameLayout.LayoutParams bgLp = new FrameLayout.LayoutParams(-1, -1);
    contentFrame.addView(backgroundView, bgLp);
    ...
    bottomNav.setPadding(padding(10), padding(8), padding(10), padding(8));
    int navBg;
if (themeMode == THEME_CUSTOM) {
    // 自定义背景模式：完全透明，让背景图片显示
    navBg = Color.TRANSPARENT;
} else {
    // 普通模式：半透明，与背景融合
    navBg = darkMode ? Color.argb(180, 14, 18, 24) : Color.argb(180, 244, 247, 250);
}
bottomNav.setBackground(round(navBg, 22, Color.TRANSPARENT, 0));
...
// ---------- 颜色 ----------
private int bg() { return darkMode ? Color.rgb(14, 18, 24) : Color.rgb(244, 247, 250); }
private int card() { return darkMode ? Color.rgb(25, 31, 39) : Color.WHITE; }
private int card2() { return darkMode ? Color.rgb(32, 39, 48) : Color.rgb(237, 241, 245); }
private int text() { return darkMode ? Color.rgb(244, 247, 250) : Color.rgb(24, 31, 38); }
private int sub() { return darkMode ? Color.rgb(166, 176, 188) : Color.rgb(92, 103, 116); }
private int line() { return darkMode ? Color.argb(78, 255, 255, 255) : Color.rgb(214, 221, 229); }

private int cardBg(boolean withAlpha) {
    if (!withAlpha) return card();
    if (themeMode == THEME_CUSTOM && backgroundImageUri != null) {
        int baseColor = darkMode ? Color.rgb(29, 34, 42) : Color.WHITE;
        int alpha = darkMode ? 185 : 215;
        return Color.argb(alpha, Color.red(baseColor), Color.green(baseColor), Color.blue(baseColor));
    }
    return card();
}

private int accentColorWithAlpha(int color) {
    if (themeMode == THEME_CUSTOM && backgroundImageUri != null) {
        int alpha = darkMode ? 205 : 225;
...
private int padding(int n) { return dp(n); }
...
private LinearLayout cardColumn() {
    ...
    l.setPadding(padding(14), padding(14), padding(14), padding(14));
    l.setBackground(round(cardBg(true), 19, line(), 1));
    return l;
}
...
private ScrollView scroll() { ScrollView s = new ScrollView(this); s.setFillViewport(true); s.setBackgroundColor(Color.TRANSPARENT); return s; }

private TextView primaryButton(String s) {
    TextView t = tv(s, TEXT_BUTTON, Color.WHITE, true);
    t.setGravity(Gravity.CENTER);
    int bgColor = accentColorWithAlpha(GREEN);
    t.setBackground(round(bgColor, 17, Color.TRANSPARENT, 0));
    return t;
}

private TextView secondaryButton(String s) {
    TextView t = tv(s, TEXT_BODY, text(), true);
    t.setGravity(Gravity.CENTER);
    t.setBackground(round(cardBg(true), 15, line(), 1));
    return t;
}
...
private EditText input(String hint, String subHint) {
    ...
    e.setTextSize(TEXT_BODY);
    e.setTextColor(text());
    e.setHintTextColor(sub());
    e.setHint(hint + "  ·  " + subHint);
    e.setPadding(padding(15), 0, padding(15), 0);
    e.setBackground(round(cardBg(true), 15, line(), 1));
    return e;
}
...
private void toggleTheme() {
    ...
    darkMode = (themeMode == THEME_DARK);
    getPreferences(MODE_PRIVATE).edit().putInt("theme_mode", themeMode).apply();
    if (themeMode != THEME_CUSTOM) {
        resetBackground();
    }
    buildShell();
...
private String getThemeName() {
    switch (themeMode) {
        ...
        case THEME_CUSTOM: return "自定义背景";
        default: return "未知";
    }
}
...
private void showPage(int page) {
    ...
    if (page != 4 && stickyFilterContainer != null) {
        hideStickyFilter();
    }
...
bottomNav.setBackground(round(navBg, 22, Color.TRANSPARENT, 0));
    ...
    for (int i = 0; i < 4; i++) {
        ...
        item.setPadding(padding(4), padding(5), padding(4), padding(5));
        int bgColor = selected ? blend(cardBg(true), GREEN, 0.12f) : Color.TRANSPARENT;
        item.setBackground(round(bgColor, 16, Color.TRANSPARENT, 0));
        item.setOnClickListener(v -> showPage(page));

        TextView icon = tv(icons[i], TEXT_ICON, selected ? GREEN : sub(), true);
        icon.setGravity(Gravity.CENTER);
        TextView label = tv(labels[i], TEXT_SMALL, selected ? GREEN : text(), selected);
...
private void buildHome() {
    ...
    box.setPadding(padding(20), padding(18), padding(20), padding(28));

    LinearLayout header = row();
    header.setGravity(Gravity.CENTER_VERTICAL);
    LinearLayout titles = column();
    TextView eyebrow = tv("MINECRAFT BEDROCK", TEXT_LABEL, GREEN, true);
    TextView title = tv("材质编辑器", TEXT_TITLE, text(), true);
    TextView subtitle = tv("在手机上创建、编辑和生成你的材质包", TEXT_SMALL, sub(), false);
    ...
    TextView theme = tv(icon, TEXT_HEADING, text(), true);
    theme.setGravity(Gravity.CENTER);
    theme.setBackground(round(cardBg(true), 50, line(), 1));
    ...
    hero.setPadding(padding(18), padding(18), padding(18), padding(18));
    int heroBg = blend(cardBg(true), GREEN, darkMode ? 0.10f : 0.06f);
    int heroBorder = darkMode ? Color.argb(60, 52, 190, 73) : blend(GREEN, bg(), 0.65f);
    hero.setBackground(round(heroBg, 22, heroBorder, 1));
    hero.addView(tv("开始制作", TEXT_LABEL, GREEN, true));
    hero.addView(tv("从一个空白项目开始", TEXT_HEADING, text(), true), marginTop(dp(4)));
    hero.addView(tv("输入 Minecraft ID，选择纹理图片，然后生成 .mcpack。", TEXT_SMALL, sub(), false), marginTop(dp(5)));
    ...
    int recentBgColor = darkMode ? Color.argb(50, 255, 165, 0) : Color.argb(30, 255, 165, 0);
    int recentBorderColor = darkMode ? Color.argb(80, 255, 165, 0) : line();

    LinearLayout recentCard = column();
    recentCard.setPadding(padding(14), padding(14), padding(14), padding(14));
    recentCard.setBackground(round(recentBgColor, 19, recentBorderColor, 1));

    LinearLayout recentHead = row();
    recentHead.addView(tv("最近项目", TEXT_HEADING, text(), true), new LinearLayout.LayoutParams(0, -2, 1));
    TextView all = tv("全部  ›", TEXT_LABEL, GREEN, true);
    ...
    // 项目状态（青色）
    int cyanBg = darkMode ? Color.argb(40, 0, 188, 212) : Color.argb(25, 0, 188, 212);
    int cyanBorder = darkMode ? Color.argb(70, 0, 188, 212) : line();
    LinearLayout infoCard = column();
    infoCard.setPadding(padding(14), padding(14), padding(14), padding(14));
    infoCard.setBackground(round(cyanBg, 19, cyanBorder, 1));
    ...
    stats.addView(statItem("版本", "1.4.3", PURPLE), new LinearLayout.LayoutParams(0, dp(70), 1));
...
private void buildEditorList() {
    ScrollView scroll = scroll();
    LinearLayout box = column();
    box.setPadding(padding(20), padding(18), padding(20), padding(26));
    box.addView(topTitle("编辑项目", "管理你保存过的材质包项目"), marginBottom(dp(14)));

    LinearLayout toolbar = cardColumn();
    toolbar.setPadding(padding(12), padding(10), padding(12), padding(10));
    LinearLayout subpackRow = row();
    subpackRow.setGravity(Gravity.CENTER_VERTICAL);
    subpackRow.addView(tv("子包", TEXT_BODY, text(), true), new LinearLayout.LayoutParams(dp(52), -2));
    ...
    switchRow.addView(tv("开关", TEXT_BODY, text(), true), new LinearLayout.LayoutParams(dp(52), -2));
    ...
    if (all.isEmpty()) {
        ...
        empty.setPadding(padding(20), padding(46), padding(20), padding(46));
        empty.addView(tv("＋", 34, GREEN, true), centerParams());
        empty.addView(tv("还没有项目", TEXT_HEADING, text(), true), marginTop(dp(8)));
        empty.addView(tv("创建并生成一个材质包后，它会自动出现在这里。", TEXT_SMALL, sub(), false), marginTop(dp(5)));
...
private void buildGenerator() {
    ...
    LinearLayout box = column();
    box.setPadding(padding(20), padding(18), padding(20), padding(170));
    box.addView(topTitle("生成材质包", "检查项目后生成可导入 Minecraft 的 .mcpack"), marginBottom(dp(14)));

    // 当前项目摘要（薄荷绿）
    int mintBg = darkMode ? Color.argb(45, 0, 200, 150) : Color.argb(25, 0, 200, 150);
    int mintBorder = darkMode ? Color.argb(70, 0, 200, 150) : line();
    LinearLayout summary = column();
    summary.setPadding(padding(14), padding(14), padding(14), padding(14));
    summary.setBackground(round(mintBg, 19, mintBorder, 1));

    summary.addView(tv("当前项目", TEXT_LABEL, sub(), false));
    String projectName = currentProject != null && currentProject.name != null && !currentProject.name.isEmpty()
            ? currentProject.name : "未命名材质包";
    summary.addView(tv(projectName, TEXT_HEADING, text(), true), marginTop(dp(3)));
    LinearLayout stats = row();
    ...
    previewCard.addView(tv("材质预览", TEXT_HEADING, text(), true), marginBottom(dp(8)));
    if (entries.isEmpty()) {
        previewCard.addView(tv("暂无材质项目，请先添加纹理。", TEXT_SMALL, sub(), false));
    } else {
        int count = Math.min(entries.size(), 6);
        for (int i = 0; i < count; i++) {
            LinearLayout entryRow = buildEntryRow(entries.get(i), i);
            entryRow.setBackground(round(cardBg(true), 15, line(), 2));
            previewCard.addView(entryRow, marginBottom(dp(6)));
        }
        if (entries.size() > 6) {
            previewCard.addView(tv("还有 " + (entries.size() - 6) + " 个材质……", TEXT_SMALL, sub(), false), marginTop(dp(4)));
        }
    }
    ...
    form.addView(tv("材质包信息", TEXT_HEADING, text(), true), marginBottom(dp(8)));
    ...
    form.addView(tv("许可证：V3C（以项目仓库中的 LICENSE 为准）", TEXT_LABEL, sub(), false));
    box.addView(form);
    ...
    bottom.setPadding(padding(12), padding(10), padding(12), padding(10));
    bottom.setBackground(round(cardBg(true), 20, line(), 1));
    TextView generateBtn = primaryButton("生成 .mcpack");
    generateBtn.setTextSize(TEXT_HEADING);
    generateBtn.setOnClickListener(v -> generate());
    bottom.addView(generateBtn, new LinearLayout.LayoutParams(-1, dp(48)));
    TextView hint = tv(entries.isEmpty() ? "请先添加至少一个材质" : "准备完成后即可生成", TEXT_LABEL, sub(), false);
...
private void buildMine() {
    ...
    // 个人资料（金色）
    int goldBg = darkMode ? Color.argb(50, 255, 193, 7) : Color.argb(25, 255, 193, 7);
    int goldBorder = darkMode ? Color.argb(70, 255, 193, 7) : line();
    LinearLayout profile = column();
    profile.setPadding(padding(14), padding(14), padding(14), padding(14));
    profile.setBackground(round(goldBg, 19, goldBorder, 1));
    ...
    profile.addView(tv("熬糕aogao", TEXT_HEADING, text(), true), marginTop(dp(6)));
    profile.addView(tv("MC 基岩版材质编辑器开发者", TEXT_SMALL, sub(), false), marginTop(dp(2)));
    box.addView(profile, marginBottom(dp(12)));

    LinearLayout settings = cardColumn();
    settings.addView(tv("设置", TEXT_HEADING, text(), true), marginBottom(dp(7)));
    addSetting(settings, "主题模式", getThemeName(), "🎨", v -> showThemePickerDialog());
    addSetting(settings, "默认材质类型", getDefaultType(), "▣", v -> showDefaultTypeDialog());
    addSetting(settings, "自定义背景", "更换应用背景图片", "🖼️", v -> showBackgroundPicker());
    ...
    // 设备性能（红色）
    int redBg = darkMode ? Color.argb(50, 255, 82, 82) : Color.argb(25, 255, 82, 82);
    int redBorder = darkMode ? Color.argb(70, 255, 82, 82) : line();
    LinearLayout perfCard = column();
    perfCard.setPadding(padding(14), padding(14), padding(14), padding(14));
    perfCard.setBackground(round(redBg, 19, redBorder, 1));
    perfCard.addView(tv("📱 设备性能检测", TEXT_HEADING, text(), true), marginBottom(dp(7)));
    perfCard.addView(tv("点击查看手机性能评估及使用建议", TEXT_SMALL, sub(), false), marginTop(dp(2)));
    TextView perfBtn = secondaryButton("查看性能报告");
    perfBtn.setOnClickListener(v -> showDeviceInfoDialog());
    perfCard.addView(perfBtn, marginTop(dp(8)));
    box.addView(perfCard, marginBottom(dp(12)));

    // 开源项目（绿色，与“开始制作”同色）
    int openBg = darkMode ? Color.argb(50, 52, 190, 73) : Color.argb(25, 52, 190, 73);
    int openBorder = darkMode ? Color.argb(70, 52, 190, 73) : line();
    LinearLayout open = column();
    open.setPadding(padding(14), padding(14), padding(14), padding(14));
    open.setBackground(round(openBg, 19, openBorder, 1));
    open.addView(tv("开源项目", TEXT_HEADING, text(), true), marginBottom(dp(7)));
    open.addView(tv("MC 基岩版材质编辑器", TEXT_BODY, text(), true));
    open.addView(tv("移动端本地材质包制作工具", TEXT_SMALL, sub(), false), marginTop(dp(2)));
    open.addView(tv("版本 1.4.3-bedrock", TEXT_SMALL, sub(), false), marginTop(dp(2)));
    open.addView(tv("许可证：V3C（以仓库 LICENSE 为准）", TEXT_SMALL, GREEN, true), marginTop(dp(7)));
    ...
    guide.addView(tv("帮助", TEXT_HEADING, text(), true), marginBottom(dp(7)));
...
private void buildEditorPage() {
    ...
    headText.addView(tv("编辑材质", TEXT_HEADING, text(), true));
    headText.addView(tv("添加纹理并管理当前项目", TEXT_SMALL, sub(), false), marginTop(dp(2)));
    head.addView(headText, new LinearLayout.LayoutParams(0, -2, 1));
    box.addView(head, marginBottom(dp(14)));

    // 添加新材质卡片（浅绿色）
    int limeBg = darkMode ? Color.argb(45, 139, 195, 74) : Color.argb(25, 139, 195, 74);
    int limeBorder = darkMode ? Color.argb(70, 139, 195, 74) : line();
    LinearLayout addCard = column();
    addCard.setPadding(padding(14), padding(14), padding(14), padding(14));
    addCard.setBackground(round(limeBg, 19, limeBorder, 1));

    addCard.addView(tv("添加新材质", TEXT_HEADING, text(), true), marginBottom(dp(8)));
    ...
    TextView quick = tv("常用 ID：minecraft:stone   minecraft:diamond   minecraft:oak_log   minecraft:diamond_sword", TEXT_LABEL, sub(), false);
    quick.setPadding(padding(4), padding(4), padding(4), padding(4));
    addCard.addView(quick, marginBottom(dp(6)));

    LinearLayout subpackRow = row();
    subpackRow.setGravity(Gravity.CENTER_VERTICAL);
    subpackRow.addView(tv("子包", TEXT_SMALL, text(), true), new LinearLayout.LayoutParams(dp(48), -2));
    ...
    preview = new ImageView(this); preview.setScaleType(ImageView.ScaleType.CENTER_INSIDE); preview.setBackground(round(cardBg(true), 18, line(), 1));
    addCard.addView(preview, new LinearLayout.LayoutParams(-1, dp(180)));
    selectedText = tv("尚未选择图片", TEXT_SMALL, sub(), false); addCard.addView(selectedText, marginTop(dp(5)));
    TextView add = primaryButton("＋  加入编辑列表"); add.setOnClickListener(v -> addEntry());
    addCard.addView(add, marginTop(dp(10)));
    status = tv("准备就绪", TEXT_SMALL, sub(), false); addCard.addView(status, marginTop(dp(6)));
    box.addView(addCard, marginBottom(dp(14)));

    LinearLayout listCard = cardColumn();
    listCard.addView(tv("当前材质", TEXT_HEADING, text(), true), marginBottom(dp(7)));

    contentFilterContainer = createFilterView(false);
    listCard.addView(contentFilterContainer, marginBottom(dp(7)));
    ...
    editorScrollView.addView(box);
    rootFrame.addView(editorScrollView, new FrameLayout.LayoutParams(-1, -1));

    stickyFilterContainer = createFilterView(true);
    stickyFilterContainer.setVisibility(View.GONE);
    stickyFilterContainer.setAlpha(0f);
    stickyFilterContainer.setTranslationY(-dp(100));
    stickyFilterContainer.setBackground(round(cardBg(true), 15, line(), 1));
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
        stickyFilterContainer.setElevation(dpF(4));
    }
    ...
    rootFrame.addView(stickyFilterContainer, stickyParams);

    editorScrollView.post(() -> {
        editorScrollView.getViewTreeObserver().addOnScrollChangedListener(() -> {
            if (contentFilterContainer == null || stickyFilterContainer == null) return;

            int[] location = new int[2];
            contentFilterContainer.getLocationInWindow(location);
            int[] scrollLoc = new int[2];
            editorScrollView.getLocationInWindow(scrollLoc);
            int relativeTop = location[1] - scrollLoc[1] - editorScrollView.getScrollY();
...
// ---------- 粘性筛选器动画 ----------
private void hideStickyFilter() {
    if (stickyFilterContainer == null) return;
    stickyFilterContainer.animate().cancel();

    if (stickyFilterContainer.getVisibility() != View.VISIBLE) return;

    stickyFilterContainer.animate()
            .translationY(-dp(80))
            .alpha(0f)
            .setDuration(200)
            .setInterpolator(new android.view.animation.AccelerateInterpolator())
            .withEndAction(() -> stickyFilterContainer.setVisibility(View.GONE))
            .start();
}

private void showStickyFilter() {
    if (stickyFilterContainer == null) return;
    stickyFilterContainer.animate().cancel();

    if (stickyFilterContainer.getVisibility() == View.VISIBLE
            && stickyFilterContainer.getTranslationY() == 0) {
        return;
    }

    stickyFilterContainer.setTranslationY(-dp(80));
    stickyFilterContainer.setAlpha(0f);
    stickyFilterContainer.setVisibility(View.VISIBLE);
...
private void showAddSubpackDialog() {
    ...
    input.setHint("输入子包名称（仅字母、数字、下划线、连字符）");
    ...
    builder.setPositiveButton("添加", (dialog, which) -> {
        ...
        if (!name.matches("^[a-zA-Z0-9_-]+$")) {
            Toast.makeText(this, "名称只能包含字母、数字、下划线、连字符", Toast.LENGTH_SHORT).show();
            return;
        }
...
private void refreshEditorList() {
    ...
    if (filteredEntries.isEmpty()) {
        String msg = (currentSubpackFilter.isEmpty() && currentSwitchFilter.isEmpty()) ?
                "暂无项目" : "没有符合条件的材质";
        editorListContainer.addView(tv(msg, TEXT_SMALL, sub(), false));
        updateEditorListTitle();
        return;
    }

    if (filteredEntries.size() <= PAGE_SIZE) {
        loadEntries(filteredEntries, 0, filteredEntries.size());
        updateEditorListTitle();
        return;
    }

    currentDisplayCount = PAGE_SIZE;
    loadEntries(filteredEntries, 0, currentDisplayCount);
    addLoadMoreButton(filteredEntries);
    updateEditorListTitle();
}

private void loadEntries(List<Entry> filteredEntries, int start, int end) {
    if (start >= filteredEntries.size()) return;
    int actualEnd = Math.min(end, filteredEntries.size());
...
private void addLoadMoreButton(List<Entry> filteredEntries) {
    if (currentDisplayCount >= filteredEntries.size()) return;
    ...
    btnContainer.setPadding(padding(12), padding(12), padding(12), padding(12));

    int remaining = filteredEntries.size() - currentDisplayCount;
    TextView loadMoreBtn = secondaryButton("加载更多 (" + remaining + " 个剩余)");
    loadMoreBtn.setOnClickListener(v -> {
        ...
        loadMoreTask = new AsyncTask<Void, Void, Void>() {
            ...
            protected void onPostExecute(Void aVoid) {
                ...
                editorListContainer.removeView(btnContainer);
                int loadCount = Math.min(PAGE_SIZE, remaining);
                loadEntries(filteredEntries, currentDisplayCount, currentDisplayCount + loadCount);
                currentDisplayCount += loadCount;
                if (currentDisplayCount < filteredEntries.size()) {
                    addLoadMoreButton(filteredEntries);
                }
                isLoadingMore = false;
                updateEditorListTitle();
    ...
    btnContainer.addView(loadMoreBtn, new LinearLayout.LayoutParams(-1, dp(48)));
    editorListContainer.addView(btnContainer);
}
...
private LinearLayout buildEntryRow(Entry e, int index) {
    ...
    card.setPadding(padding(10), padding(8), padding(8), padding(8));
    card.setBackground(round(cardBg(true), 15, line(), 1));

    ImageView img = new ImageView(this);
    img.setScaleType(ImageView.ScaleType.CENTER_CROP);
    img.setBackground(round(cardBg(true), 12, line(), 1));
    int targetSize = dp(56);
    Bitmap thumbnail = getThumbnail(e.imagePath, targetSize, targetSize);
    if (thumbnail != null) {
    ...
    info.addView(tv(e.type + "  " + e.id + subInfo + switchInfo, TEXT_BODY, text(), true));

    LinearLayout buttons = row();
    buttons.setGravity(Gravity.CENTER_VERTICAL);
    TextView edit = tv("✏ 修改", TEXT_SMALL, GREEN, true);
    edit.setPadding(padding(8), padding(4), padding(8), padding(4));
    edit.setBackground(round(blend(cardBg(true), GREEN, 0.15f), 10, Color.TRANSPARENT, 0));
    edit.setOnClickListener(v -> openEditEntryDialog(index));
    buttons.addView(edit);

    TextView del = tv("🗑 删除", TEXT_SMALL, Color.rgb(255, 80, 80), true);
    del.setPadding(padding(8), padding(4), padding(8), padding(4));
    del.setBackground(round(blend(cardBg(true), Color.rgb(255, 80, 80), 0.15f), 10, Color.TRANSPARENT, 0));
...
private void openEditEntryDialog(int index) {
    ...
    root.setPadding(padding(20), padding(18), padding(20), padding(18));
    root.setBackgroundColor(bg());

    root.addView(tv("修改材质", TEXT_HEADING, text(), true), marginBottom(dp(12)));
    ...
    idEdit.setTextSize(TEXT_BODY);
    idEdit.setTextColor(text());
    idEdit.setHintTextColor(sub());
    idEdit.setHint("Minecraft ID");
    idEdit.setPadding(padding(15), 0, padding(15), 0);
    idEdit.setBackground(round(cardBg(true), 15, line(), 1));
    ...
    currentEditImage.setBackground(round(cardBg(true), 18, line(), 1));
...
private LinearLayout createFilterView(boolean isSticky) {
    ...
    container.setPadding(0, padding(8), 0, padding(8));

    LinearLayout subpackRow = row();
    subpackRow.setGravity(Gravity.CENTER_VERTICAL);
    TextView subpackLabel = tv("子包筛选:", TEXT_BODY, text(), true);
    ...
    LinearLayout switchRow = row();
    switchRow.setGravity(Gravity.CENTER_VERTICAL);
    TextView switchLabel = tv("开关筛选:", TEXT_BODY, text(), true);
...
// ---------- 缓存缩略图 ----------
private Bitmap getThumbnail(String imagePath, int targetWidth, int targetHeight) {
    if (imagePath == null || imagePath.isEmpty() || mMemoryCache == null) return null;
    String cacheKey = imagePath + "_" + targetWidth + "_" + targetHeight;
    Bitmap cached = mMemoryCache.get(cacheKey);
...
// ---------- 快速模糊算法 ----------
private Bitmap fastBlur(Bitmap src, float radius) {
    ...
    int scale = 4;
    int smallW = Math.max(1, width / scale);
    int smallH = Math.max(1, height / scale);
    int[] smallPix = new int[smallW * smallH];
    for (int y = 0; y < smallH; y++) {
        for (int x = 0; x < smallW; x++) {
            ...
            smallPix[y * smallW + x] = pixels[srcY * width + srcX];
        }
    }
    int radiusInt = Math.round(radius);
    for (int pass = 0; pass < 2; pass++) {
        int[] temp = new int[smallPix.length];
        for (int y = 0; y < smallH; y++) {
            for (int x = 0; x < smallW; x++) {
                ...
                temp[y*smallW + x] = Color.rgb(r/count, g/count, b/count);
            }
        }
        smallPix = temp;
    }
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            outPixels[y*width + x] = smallPix[srcY*smallW + srcX];
        }
    }
...
// ---------- 自定义背景 ----------
private boolean loadBackgroundIntoView(Uri uri) {
    if (uri == null || backgroundView == null) return false;
    try {
        Bitmap bmp = decodeBackgroundBitmap(uri);
        if (bmp == null) return false;
        cachedBackgroundBitmap = bmp;
...
private Bitmap decodeBackgroundBitmap(Uri uri) throws Exception {
    BitmapFactory.Options options = new BitmapFactory.Options();
    options.inJustDecodeBounds = true;
    ...
    int maxSize = Math.max(sw, sh) * 2;
    int sample = 1;
    while (options.outWidth / sample > maxSize || options.outHeight / sample > maxSize) sample *= 2;
    options.inJustDecodeBounds = false;
...
private Uri saveBackgroundPermanently(Uri sourceUri) throws Exception {
    File dir = new File(getFilesDir(), "custom_background");
    if (!dir.exists() && !dir.mkdirs()) throw new IOException("无法创建背景目录");
    File out = new File(dir, "background.png");
...
private boolean applyBackground(Uri uri) {
    if (uri == null) return false;
    try {
        Uri permanentUri = uri;
        if (!"file".equals(uri.getScheme())) {
            permanentUri = saveBackgroundPermanently(uri);
        }
        backgroundImageUri = permanentUri;
        cachedBackgroundBitmap = null;
        themeMode = THEME_CUSTOM;
        darkMode = false;
        getPreferences(MODE_PRIVATE).edit()
                .putInt("theme_mode", THEME_CUSTOM)
                .putString("bg_uri", permanentUri.toString())
                .apply();
        return loadBackgroundIntoView(permanentUri);
    } catch (Exception e) {
        e.printStackTrace();
        return false;
    }
...
private void resetBackground() {
    ...
    if (pageHost != null) pageHost.setBackgroundColor(Color.TRANSPARENT);
    backgroundImageUri = null;
    cachedBackgroundBitmap = null;
    getPreferences(MODE_PRIVATE).edit().remove("bg_uri").apply();
    Toast.makeText(this, "已恢复默认背景", Toast.LENGTH_SHORT).show();
}

private void showBackgroundPicker() {
    String[] options = {"📷 选择并裁剪", "❌ 恢复默认"};
    new AlertDialog.Builder(this)
            .setTitle("自定义背景")
            .setItems(options, (dialog, which) -> {
                if (which == 0) {
                    ...
                    startActivityForResult(intent, BG_PICK_IMAGE);
                } else {
                    resetBackground();
                    showPage(currentPage);
                    Toast.makeText(this, "已恢复默认背景", Toast.LENGTH_SHORT).show();
...
// ---------- 系统裁剪（自由比例） ----------
private void showCropDialog(final Uri uri, final int mode) {
    new AlertDialog.Builder(this)
            .setTitle("裁剪图片")
            .setMessage("是否对图片进行裁剪？\n（若选择“直接使用”将保留原图）")
            .setPositiveButton("裁剪", (dialog, which) -> startSystemCrop(uri, mode))
            .setNegativeButton("直接使用", (dialog, which) -> handleImageSelected(uri, mode))
            .show();
}

private void startSystemCrop(Uri sourceUri, int mode) {
    try {
        File destFile = new File(getCacheDir(), "cropped_" + System.currentTimeMillis() + ".png");
        if (!destFile.getParentFile().exists()) destFile.getParentFile().mkdirs();
        Uri destinationUri = FileProvider.getUriForFile(this,
                getPackageName() + ".fileprovider", destFile);
        cropOutputUri = destinationUri;
        ...
        startActivityForResult(intent, SYSTEM_CROP_REQUEST);
        cropMode = mode;
    } catch (ActivityNotFoundException e) {
        Toast.makeText(this, "系统裁剪功能不可用，将直接使用原图", Toast.LENGTH_LONG).show();
        handleImageSelected(sourceUri, mode);
    } catch (Exception e) {
        Toast.makeText(this, "启动裁剪失败：" + e.getMessage(), Toast.LENGTH_LONG).show();
        handleImageSelected(sourceUri, mode);
    }
}

private void handleImageSelected(Uri uri, int mode) {
    if (mode == 0) {
        selectedUri = uri;
        try {
        ...
        boolean success = applyBackground(uri);
        if (success) {
...
private void addRecentRows(LinearLayout parent, int max) {
    ...
    if (projects.isEmpty()) {
        parent.addView(tv("还没有项目。点击“创建材质”开始你的第一个作品。", TEXT_SMALL, sub(), false), marginTop(dp(6)));
        return;
    }
...
private void addProjectRow(LinearLayout parent, Project p) {
    ...
    row.setPadding(padding(12), padding(10), padding(10), padding(10));
    row.setBackground(round(cardBg(true), 15, line(), 1));
    ...
    LinearLayout info = column();
    info.addView(tv(p.name, TEXT_BODY, text(), true));
    info.addView(tv(p.date + "   " + p.entries.size() + " 个材质", TEXT_SMALL, sub(), false), marginTop(dp(3)));
    row.addView(info, new LinearLayout.LayoutParams(0, -2, 1));

    TextView state = tv("打开", TEXT_LABEL, GREEN, true);
    state.setGravity(Gravity.CENTER);
    state.setPadding(padding(10), 0, padding(10), 0);
...
private void addGuide(LinearLayout parent, String iconText, String title,
    ...
    r.setPadding(padding(10), padding(8), padding(8), padding(8));
    r.setBackground(round(cardBg(true), 13, line(), 1));

    TextView icon = tv(iconText, TEXT_HEADING, accent, true);
    icon.setGravity(Gravity.CENTER);
    r.addView(icon, new LinearLayout.LayoutParams(dp(42), dp(42)));

    LinearLayout info = column();
    info.addView(tv(title, TEXT_BODY, text(), true));
    info.addView(tv(desc, TEXT_LABEL, sub(), false), marginTop(dp(2)));
    r.addView(info, new LinearLayout.LayoutParams(0, -2, 1));

    TextView arrow = tv("›", TEXT_HEADING, sub(), false);
...
private void addSetting(LinearLayout parent, String title, String value, String icon, View.OnClickListener click) {
    LinearLayout r = row(); r.setGravity(Gravity.CENTER_VERTICAL); r.setPadding(padding(5), padding(9), padding(5), padding(9));
    TextView ic = tv(icon, TEXT_HEADING, GREEN, true); ic.setGravity(Gravity.CENTER);
    r.addView(ic, new LinearLayout.LayoutParams(dp(42), dp(44)));
    LinearLayout info = column(); info.addView(tv(title, TEXT_BODY, text(), true)); info.addView(tv(value, TEXT_SMALL, sub(), false), marginTop(dp(2)));
    r.addView(info, new LinearLayout.LayoutParams(0, -2, 1));
    r.addView(tv("›", TEXT_HEADING, sub(), false));
    r.setOnClickListener(click); parent.addView(r, marginBottom(dp(2)));
}

// actionCard 已包含颜色，无需修改
private LinearLayout actionCard(String icon, String title, String desc, int accent, View.OnClickListener click) {
    LinearLayout c = cardColumn();
    c.setGravity(Gravity.CENTER);
    c.setPadding(padding(8), padding(14), padding(8), padding(12));
    c.setOnClickListener(click);

    int bgColor;
    if (darkMode) {
        bgColor = Color.argb(60, Color.red(accent), Color.green(accent), Color.blue(accent));
    } else {
        bgColor = Color.argb(40, Color.red(accent), Color.green(accent), Color.blue(accent));
    }
    int borderColor = darkMode ? Color.argb(80, Color.red(accent), Color.green(accent), Color.blue(accent)) : line();
    c.setBackground(round(bgColor, 18, borderColor, 1));

    TextView ic = tv(icon, 32, accent, true);
    ic.setGravity(Gravity.CENTER);
    c.addView(ic, new LinearLayout.LayoutParams(-1, dp(42)));
    c.addView(tv(title, TEXT_BODY, text(), true), centerParams());
    c.addView(tv(desc, TEXT_SMALL, sub(), false), centerMarginParams(dp(4)));
...
private LinearLayout statItem(String label, String value, int accent) {
    ...
    box.setPadding(padding(12), padding(8), padding(12), padding(6));
    box.setBackground(round(cardBg(true), 14, line(), 1));
    box.addView(tv(value, TEXT_HEADING, accent, true), centerParams());
    box.addView(tv(label, TEXT_LABEL, sub(), false), centerMarginParams(dp(2)));
    return box;
}
...
private LinearLayout sectionTitle(String title, String desc) {
    LinearLayout l = column();
    l.addView(tv(title, TEXT_HEADING, text(), true));
    l.addView(tv(desc, TEXT_SMALL, sub(), false), marginTop(dp(3)));
    return l;
}

private LinearLayout topTitle(String title, String desc) {
    LinearLayout l = column();
    l.addView(tv(title, TEXT_TITLE, text(), true));
    l.addView(tv(desc, TEXT_SMALL, sub(), false), marginTop(dp(4)));
    return l;
}
...
private void showChangelog() {
    new AlertDialog.Builder(this).setTitle("更新日志")
            .setMessage("1.4.3\n• 裁剪改为自由比例（aspectX/Y=0）\n• 主题切换对话框（浅色/深色/自定义）\n• 深色模式卡片发光效果（白色微光）\n• 快速入口卡片分别发出绿色、蓝色、紫色微光\n• 优化主题切换卡顿（背景缓存）")
...
protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (requestCode == SYSTEM_CROP_REQUEST) {
        if (resultCode == RESULT_OK && cropOutputUri != null) {
            handleImageSelected(cropOutputUri, cropMode);
    ...
    if (requestCode == PICK_IMAGE && resultCode == RESULT_OK && data != null) {
        ...
        if (uri != null) {
            showCropDialog(uri, 0);
        }
        return;
    }

    if (requestCode == BG_PICK_IMAGE && resultCode == RESULT_OK && data != null) {
        Uri uri = data.getData();
        if (uri != null) {
            showCropDialog(uri, 1);
        }
        return;
    }
...
private interface BuildProgressListener {
    ...
    private void generate() {
        ...
        new AsyncTask<Void, Integer, Exception>() {
            ...
            protected void onProgressUpdate(Integer... values) {
                ...
                String message;
                if (p < 5) message = "准备工作目录  " + p + "%";
                else if (p < 60) message = "正在处理纹理  " + p + "%";
                else if (p < 70) message = "材质文件准备完成  " + p + "%";
                else if (p < 100) message = "正在压缩材质包  " + p + "%";
                else message = "生成完成  " + p + "%";
    ...
    private void buildBedrock(File root, String author, String description, BuildProgressListener progress) throws Exception {
        ...
        String packDisplayName = packNameInput != null ? packNameInput.getText().toString().trim() : "我的材质包";
        if (packDisplayName.isEmpty()) packDisplayName = "我的材质包";

        StringBuilder manifest = new StringBuilder();
        manifest.append("{\n");
        manifest.append("  \"format_version\": 3,\n");
        manifest.append("  \"header\": {\n");
        manifest.append("    \"name\": \"" + json(packDisplayName) + "\",\n");
    ...
    private void copyAsPng(File srcFile, File out) throws Exception {
        ...
        options.inSampleSize = sample;
        options.inJustDecodeBounds = false;
        Bitmap b = BitmapFactory.decodeFile(srcFile.getAbsolutePath(), options);
    ...
    private String getDeviceInfo() {
        ...
        info.append("📱 设备型号：").append(android.os.Build.MANUFACTURER)
            .append(" ").append(android.os.Build.MODEL).append("\n");
        info.append("📲 系统版本：Android ").append(android.os.Build.VERSION.RELEASE)
            .append(" (API ").append(android.os.Build.VERSION.SDK_INT).append(")\n");

        info.append("\n🧠 处理器信息：\n");
        String arch = System.getProperty("os.arch");
        info.append("  架构：").append(arch != null ? arch : "未知").append("\n");

        int cores = Runtime.getRuntime().availableProcessors();
        info.append("  核心数：").append(cores).append(" 核\n");

        String hardware = android.os.Build.HARDWARE;
        if (hardware != null && !hardware.isEmpty()) {
            info.append("  硬件平台：").append(hardware).append("\n");
        }
        String soc = android.os.Build.SOC_MODEL;
        if (soc != null && !soc.isEmpty()) {
            info.append("  SoC 型号：").append(soc).append("\n");
        }

        String maxFreq = readCpuMaxFreq();
        if (maxFreq != null) {
            info.append("  最高频率：").append(maxFreq).append(" MHz\n");
        }

        for (String abi : android.os.Build.SUPPORTED_64_BIT_ABIS) {
            if (abi != null && !abi.isEmpty()) {
                info.append("  架构：64 位 (").append(abi).append(")\n");
                break;
            }
        }
        ...
        if (am != null) {
            ...
            info.append("  总运存：").append(formatSize(totalMem)).append("\n");
            info.append("  可用运存：").append(formatSize(availMem)).append("\n");
        }
        ...
        long availStorage = availableBlocks * blockSize;
        info.append("  总存储：").append(formatSize(totalStorage)).append("\n");
        info.append("  可用存储：").append(formatSize(availStorage)).append("\n");

        long totalMemGB = memInfo.totalMem / (1024 * 1024 * 1024);
        int score = 0;

        if (totalMemGB >= 8) score += 40;
        else if (totalMemGB >= 6) score += 30;
        else if (totalMemGB >= 4) score += 20;
        else score += 10;
        ...
        for (String abi : android.os.Build.SUPPORTED_64_BIT_ABIS) {
            if (abi != null && !abi.isEmpty()) { is64bit = true; break; }
        }
        if (is64bit) score += 20;
        else score += 10;

        String hw = (hardware != null) ? hardware.toLowerCase() : "";
        String socModel = (soc != null) ? soc.toLowerCase() : "";
        if (hw.contains("qcom") || socModel.contains("snapdragon")) score += 10;
        else if (hw.contains("exynos") || socModel.contains("exynos")) score += 8;
        else if (hw.contains("mtk") || socModel.contains("mediatek") || hw.contains("k67") || hw.contains("k68")) {
            if (socModel.contains("dimensity")) score += 8;
            else score += 5;
        }

        long availGB = memInfo.availMem / (1024 * 1024 * 1024);
        if (availGB >= 3) score += 10;
        else if (availGB >= 1.5) score += 5;
        ...
        if (score >= 80) {
            grade = "⭐⭐⭐ 旗舰级（高性能）";
            advice = "可以流畅运行绝大多数材质包。\n建议：可放心使用大型材质包（200 MB+）";
        } else if (score >= 60) {
            grade = "⭐⭐ 中高端";
            advice = "可以运行大多数材质包。\n建议：推荐使用 50-150 MB 的材质包\n如果遇到卡顿，可适当降低纹理分辨率";
        } else if (score >= 40) {
            grade = "⭐ 中端";
            advice = "可以运行中小型材质包。\n建议：使用 < 50 MB 的材质包\n避免导入包含大量模型的复杂包";
        } else {
            grade = "入门级";
            advice = "建议只使用轻量级材质包（< 20 MB）。\n请勿尝试导入包含大量纹理或模型的大型包\n否则可能导致应用卡顿或无响应";
        }
        info.append("  性能评分：").append(score).append(" 分\n");
        info.append("  等级：").append(grade).append("\n");
        info.append("\n💡 建议：").append(advice).append("\n");

        if (score < 40) {
            info.append("\n⚠️ 注意：你的设备性能较低，强烈建议不要导入 \"Actions & Stuff\"\n");
            info.append("  等大型模组材质包，以免严重卡顿或崩溃。");
        }
                    ...
                    if (path.endsWith("cpuinfo")) {
                        ...
                        while ((line = br.readLine()) != null) {
                            if (line.startsWith("cpu MHz")) {
                                ...
                                if (parts.length >= 2) {
                                    float mhz = Float.parseFloat(parts[1].trim());
                                    br.close();
                                    return String.format("%.2f", mhz);
                                }
                            }
                        }
                        br.close();
                        ...
                        if (line != null) {
                            ...
                            return String.format("%.2f", freq / 1000000.0);
                        }
                    }
                }
            }
    ...
    private void showDeviceInfoDialog() {
        ...
        builder.setNegativeButton("刷新", (dialog, which) -> showDeviceInfoDialog());
        builder.show();
    }
    ...
    private void showThemePickerDialog() {
        final String[] themes = {"浅色", "深色", "自定义背景"};
        int checked = 0;
        if (themeMode == THEME_LIGHT) checked = 0;
        else if (themeMode == THEME_DARK) checked = 1;
        else if (themeMode == THEME_CUSTOM) checked = 2;

        new AlertDialog.Builder(this)
                .setTitle("选择主题模式")
                .setSingleChoiceItems(themes, checked, (dialog, which) -> {
                    switch (which) {
                        case 0: themeMode = THEME_LIGHT; break;
                        case 1: themeMode = THEME_DARK; break;
                        case 2: themeMode = THEME_CUSTOM; break;
                    }
                    darkMode = (themeMode == THEME_DARK);
                    getPreferences(MODE_PRIVATE).edit().putInt("theme_mode", themeMode).apply();
                    if (themeMode == THEME_CUSTOM) {
                        if (backgroundImageUri != null) {
                            getPreferences(MODE_PRIVATE).edit().putString("bg_uri", backgroundImageUri.toString()).apply();
                        }
                    } else {
                        getPreferences(MODE_PRIVATE).edit().remove("bg_uri").apply();
                        backgroundImageUri = null;
