...
@Override protected void onCreate(Bundle b) {
    ...
    private void refreshBottomNav() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            TextView label = tv(labels[i], 12, i == currentPage ? GREEN : text(), false);
...
private void buildHome() {
    ...
    actions.addView(actionCard("＋", "创建材质", "从零开始制作", GREEN, v -> {
        currentProject = null;
        entries.clear();
        subpackList.clear();      // ✅ 清空子包列表
        saveSubpackList();        // ✅ 保存清空后的状态
        showPage(4);
    }), equalActionParams(0));
    ...
    addGuide(guide, "↻", "如何替换物品纹理", "查看详细教程", BLUE, v -> showTextureGuide());
    addGuide(guide, "▣", "材质包结构说明", "了解文件结构", PURPLE, v -> showPackStructure());
    addGuide(guide, "?", "常见问题解答", "解决你的疑惑", Color.rgb(238, 167, 31), v -> showFAQ());
...
private void buildGenerator() {
    ...
    // 显示所有独立开关（自动收集）
    LinearLayout switchSummary = cardColumn();
    switchSummary.addView(tv("独立启用开关列表", 16, text(), true), marginBottom(dp(6)));
    StringBuilder sw = new StringBuilder();
    Set<String> switchNames = new HashSet<>();
    for (Entry e : entries) {
        if (e.independentEnabled && !e.switchName.isEmpty()) {
            switchNames.add(e.switchName);
        }
    }
    ...
    TextView swInfo = tv(sw.toString(), 13, sub(), false);
    switchSummary.addView(swInfo);
    box.addView(switchSummary, marginBottom(dp(12)));

    LinearLayout subpackInfo = cardColumn();
    subpackInfo.addView(tv("当前子包列表", 16, text(), true), marginBottom(dp(6)));
    StringBuilder sb = new StringBuilder();
    if (subpackList.isEmpty()) {
        sb.append("（无子包，所有材质默认在根包）");
    } else {
        for (String s : subpackList) {
            sb.append("• ").append(s).append("\n");
        }
    }
    TextView info = tv(sb.toString(), 13, sub(), false);
    subpackInfo.addView(info);
    box.addView(subpackInfo, marginBottom(dp(12)));
...
private void buildMine() {
    ...
    profile.addView(profileIcon, profileIconParams);
    ...
    addSetting(settings, "用户协议", "查看完整用户协议及免责声明", "📄", v -> showFullDisclaimerDialog());
...
private void buildEditorPage() {
    ...
    TextView subLabel = tv("子包:", 14, text(), true);
    subpackRow.addView(subLabel, new LinearLayout.LayoutParams(-2, -2));
    subpackSpinner = new Spinner(this);
    List<String> spinnerItems = new ArrayList<>();
    spinnerItems.add("默认/根包");
    spinnerItems.addAll(subpackList);
    subpackAdapter = createSpinnerAdapter(spinnerItems);
    subpackSpinner.setAdapter(subpackAdapter);
    setupSpinnerPopup(subpackSpinner);
    subpackRow.addView(subpackSpinner, new LinearLayout.LayoutParams(0, -2, 1));
    box.addView(subpackRow, marginBottom(dp(8)));

    // 独立启用开关
    LinearLayout switchRow = row();
    switchRow.setGravity(Gravity.CENTER_VERTICAL);
    independentCheckBox = new CheckBox(this);
    independentCheckBox.setText("独立启用");
    independentCheckBox.setTextColor(text());
    switchRow.addView(independentCheckBox);

    switchNameInput = input("开关名称", "例如：显示钻石剑");
    switchNameInput.setVisibility(View.GONE);
    switchRow.addView(switchNameInput, new LinearLayout.LayoutParams(0, -2, 1));

    independentCheckBox.setOnCheckedChangeListener((buttonView, isChecked) -> {
        switchNameInput.setVisibility(isChecked ? View.VISIBLE : View.GONE);
    });
    box.addView(switchRow, marginBottom(dp(8)));
...
private void addProjectRow(LinearLayout parent, Project p) {
    ...
    row.setOnClickListener(v -> {
        ...
        // 加载项目时恢复子包列表（如果项目有存储的话，但目前未存储，暂时保持为空）
        showPage(4);
    });
...
private void showAgreementDialog() {
    ...
    // ✅ 关键修复：设置背景色为卡片背景，深色模式下为深色，浅色模式下为浅色
    ...
    for (File f : files) {
        if (f.isDirectory()) {
            ...
            try {
                ...
                // 在提取目录中搜索纹理文件
                File extractRoot = dir.getParentFile().getParentFile(); // models 在根目录
...
private void generate() {
    ...
    try {
        ...
        save.setType("application/octet-stream"); // 避免系统自动添加 .zip 后缀
    ...
    private void buildBedrock(File root, String author, String description) throws Exception {
        ...
        // 生成 item_texture.json 和 terrain_texture.json
        ...
        // 生成 manifest.json（format_version: 3）
