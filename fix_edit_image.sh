...
# 备份（如果已经备份过，可以跳过，但再备份一次也无妨）
BACKUP="$TARGET.bak_fix_$(date +%Y%m%d_%H%M%S)"
cp "$TARGET" "$BACKUP"
echo "✅ 已备份到：$BACKUP"

# 检查 perl
if ! command -v perl >/dev/null 2>&1; then
    echo "❌ 需要 perl，请先运行 pkg install perl"
    exit 1
fi
...
if ! grep -q "EDIT_PICK_IMAGE" "$TARGET"; then
    ...
    echo "ℹ️ 常量 EDIT_PICK_IMAGE 已存在，跳过"
fi
...
# 3. 替换 openEditEntryDialog 方法（如果还没有“重新选择图片”按钮）
if ! grep -q "重新选择图片" "$TARGET"; then
    # 在当前目录创建临时文件，避免 /tmp 权限问题
    cat > old_method.txt <<'OLD'
...
NEW
    ...
    # 使用 perl 替换
    perl -0pi -e 'BEGIN{open my $old, "<", "old_method.txt"; local $/; $old_content = <$old>; close $old; open my $new, "<", "new_method.txt"; $new_content = <$new>; close $new;} s/\Q$old_content\E/$new_content/s' "$TARGET"
    rm -f old_method.txt new_method.txt
    echo "✅ openEditEntryDialog 方法已替换"
else
    echo "ℹ️ “重新选择图片”已存在，方法替换跳过"
fi

# 4. 在 onActivityResult 中添加处理（如果不存在）
if ! grep -q "requestCode == EDIT_PICK_IMAGE" "$TARGET"; then
    ...
    echo "ℹ️ onActivityResult 处理已存在，跳过"
fi
...
echo "========== 最终验证 =========="
if grep -q "EDIT_PICK_IMAGE" "$TARGET"; then
    echo "✅ 常量 EDIT_PICK_IMAGE"
else
    echo "❌ 常量 EDIT_PICK_IMAGE 缺失"
fi
if grep -q "currentEditDialog" "$TARGET"; then
    echo "✅ 成员变量 currentEditDialog"
else
    echo "❌ 成员变量 currentEditDialog 缺失"
fi
if grep -q "重新选择图片" "$TARGET"; then
    echo "✅ 按钮“重新选择图片”"
else
...
if grep -q "requestCode == EDIT_PICK_IMAGE" "$TARGET"; then
    echo "✅ onActivityResult 处理"
else
    echo "❌ onActivityResult 处理缺失"
fi
echo ""
echo "🎉 所有修改完成！"
